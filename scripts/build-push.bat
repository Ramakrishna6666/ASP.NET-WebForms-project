...
REM Build and Push Script for FIlms Application (Windows)
REM This script builds the Docker image and pushes it to the selected registry

echo ========================================
echo FIlms Docker Build and Push Script
echo ========================================
echo.
...
REM Sanitize tag
powershell -Command "$tag='%IMAGE_TAG%'; $tag=$tag.ToLower(); $tag=$tag -replace '[^a-z0-9._\-]','-'; $tag=$tag -replace '^-+',''; $tag=$tag -replace '-+$',''; Write-Output $tag" > temp_tag.txt
